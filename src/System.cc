...
// kubeedge: Add communication protocol parameters: websocket or tcp
System::System(const string &strVocFile, const string &strSettingsFile, std::string rt, const string &strCommProtocol, const eSensor sensor, const bool bUseViewer):
    mSensor(sensor), RunType(std::move(rt)), mpViewer(static_cast<Viewer*>(NULL)), mbReset(false),mbActivateLocalizationMode(false),
    mbDeactivateLocalizationMode(false)
{
    // Output welcome message
    ...
    if (RunType.compare("client") == 0){
        ...
        //(it will live in the main thread of execution, the one that called this constructor)
        mpTracker = new Tracking(this, mpVocabulary, mpFrameDrawer, mpMapDrawer,
                                mpMap, mpKeyFrameDatabase, strSettingsFile, strCommProtocol, mSensor);
    } else if (RunType.compare("server") == 0){
        // Edge-SLAM: added settings file
        //Initialize the Local Mapping thread and launch
        mpLocalMapper = new LocalMapping(mpMap, mpKeyFrameDatabase, mpVocabulary, strSettingsFile, strCommProtocol, mSensor==MONOCULAR);
        mptLocalMapping = new thread(&ORB_SLAM2::LocalMapping::Run,mpLocalMapper);
...
void System::ServerShutdown(bool save_traj)
{
    mpLocalMapper->RequestFinish();
    ...
    while(!mpLocalMapper->isFinished() || !mpLoopCloser->isFinished() || mpLoopCloser->isRunningGBA())
    {
        if(save_traj && !has_saved)
        {
            SaveKeyFrameTrajectoryTUM("/KeyFrameTrajectory-edgeslam.txt");
            has_saved = true;
        }
        usleep(1000);
    }
